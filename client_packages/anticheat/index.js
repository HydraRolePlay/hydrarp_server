...
mp.events.add("client:checkInvincible", () => {
	...
	setTimeout(() => {
		if (healthBefore == Behaviour.health) {
			if (mp.players.local.getHealth() > 0 && global.localplayer.data.login && global.localplayer.data.admin == 0) {
...
mp.events.add('playerWeaponShot', () => {
	if(Behaviour.checkWeaponhash() && global.localplayer.data.login && global.localplayer.data.admin == 0) {
		mp.events.callRemote("server:CheatDetection", "Unallowed Weapon")
	}
	if(Behaviour.reloadingWeapon && global.localplayer.data.login && global.localplayer.data.admin == 0) {
...
mp.events.add('client:respawning', () => {
	if(Behaviour.active) Behaviour.sleep(10)
})
...
mp.events.add("render", () => {
	...
	{	
		if (Behaviour.active) {
			...
			if ((Math.abs(Difference.x) > 30 || Math.abs(Difference.y) > 30) && global.localplayer.data.login && global.localplayer.data.admin == 0) {
				if(Behaviour.isWalking()) {
					mp.events.callRemote("server:CheatDetection", "Flyhack/Teleport")
				}
			}
			if (mp.players.local.vehicle && global.localplayer.data.login && global.localplayer.data.admin == 0) {
				if(Behaviour.checkCarPos(25)) {
					mp.events.callRemote("server:CheatDetection", "Vehicle Flyhack")
				}
				if(Behaviour.VehicleFasterThan(400)) {
...
setInterval(() => {
	...
	setTimeout(() => {
		if(hp<Behaviour.health && Behaviour.active && global.localplayer.data.login && global.localplayer.data.admin == 0) {
